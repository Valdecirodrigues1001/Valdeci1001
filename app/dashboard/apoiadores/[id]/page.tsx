...
const supporterStatusLabels: Record<string, string> = {
...
let assignedMemberJobTitle: string | null = null;
...
if (supporter.assigned_member_id) {
  const { data: assignedMember } = await supabase
    ...
    .select(`
      ...
      job_title
    `)
    .eq("id", supporter.assigned_member_id)
    .eq("campaign_id", membership.campaign_id)
    .maybeSingle();
  ...
  if (assignedMember) {
    const { data: assignedProfile } = await supabase
      ...
      .select(`
        full_name
      `)
      .eq("id", assignedMember.user_id)
      .maybeSingle();

    assignedMemberName =
      assignedProfile?.full_name ?? "Integrante";
...
const crmStageLabel =
  crmStageLabels[supporter.crm_stage] ??
  "Novo contato";

const supporterStatusLabel =
  supporterStatusLabels[supporter.status] ??
  "Não definido";
...
const nextContactLabel = supporter.next_contact_at
  ? new Intl.DateTimeFormat("pt-BR", {
      day: "2-digit",
      month: "2-digit",
      year: "numeric",
    }).format(new Date(supporter.next_contact_at))
  : "Não agendado";

const lastContactLabel = supporter.last_contact_at
  ? new Intl.DateTimeFormat("pt-BR", {
      day: "2-digit",
      month: "2-digit",
      year: "numeric",
    }).format(new Date(supporter.last_contact_at))
  : "Sem registro";

  const originLabel =
  originLabels[supporter.origin] ?? "Não informada";
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="mt-2 text-white/60">
                  {supporter.profession || "Profissão não informada"}
                </p>
              </div>
            </div>
  ...
  <div className="rounded-2xl bg-white/10 px-4 py-3">
    ...
    <p className="mt-1 text-sm font-medium">
      {assignedMemberName ?? "Não definido"}
    </p>
  </div>
      ...
      {nextContactLabel}
        ...
        <div className="mt-8 grid gap-8 xl:grid-cols-[1.4fr_0.8fr]">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <div className="mt-8">
              <SupporterEditForm supporter={supporter} />
            </div>
          </section>
          ...
          <div className="space-y-8">
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Registre contatos, pedidos e participações.
              </p>

              <div className="mt-6">
                <ActivityForm supporterId={supporter.id} />
              </div>
            </section>
  ...
  <div>
    ...
    <p className="mt-1 text-sm text-slate-500">
      Visão estratégica deste apoiador.
    </p>
  </div>
